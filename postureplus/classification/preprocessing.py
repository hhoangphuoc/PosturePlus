...
#               Detect landmarks in each images and write it to the csv files
    ...
    def all_landmarks_as_dataframe(self):
        ...
        total_df = None
        for class_index, class_name in enumerate(self.__pose_class_names):
            csv_out_path = os.path.join(self._csvs_out_folder_per_class,
                                        class_name + '.csv'
                                        )
...
images_in_folder = os.path.join('exercise_poses', 'train')
...
images_in_folder = os.path.join('exercise_poses', 'test')
